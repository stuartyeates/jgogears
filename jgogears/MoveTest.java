...
public class MoveTest extends TestCase {
	...
	public void testGTPConstructor() throws ParseException {
		...
		assertTrue(new Move("w resign").getColour() == Board.VERTEX_WHITE);
		...
		assertTrue(new Move("white t19").getColour() == Board.VERTEX_WHITE);
		...
		assertTrue(new Move("black t19").getColour() == Board.VERTEX_BLACK);
		...
		assertTrue(new Move("black q10").getColour() == Board.VERTEX_BLACK);
	...
	public void testStringConversions() throws ParseException {
		for (int i = 0; i < 25; i++) {
			for (int j = 0; j < 25; j++) {
				Move move = new Move(i, j, Board.VERTEX_WHITE);
				assertNotNull(move);
				assertTrue("" + i + " " + j, move.getRow() == i);
				assertTrue("" + i + " " + j, move.getColumn() == j);
				assertTrue(move.getColour() == Board.VERTEX_WHITE);
				...
				assertTrue("" + i + " " + j + " " + move + " " + move2,
						move2.getColumn() == j);
				assertTrue(move2.getColour() == Board.VERTEX_WHITE);
			}
		}
	}
	...
	public void testToString() throws ParseException {
		System.err.println(new Move(0, 0, Board.VERTEX_WHITE));
		assertTrue(new Move(0, 0, Board.VERTEX_WHITE).toString().compareTo(
				"white a1") == 0);
		assertTrue(new Move(18, 18, Board.VERTEX_WHITE).toString().compareTo(
				"white t19") == 0);
	}
